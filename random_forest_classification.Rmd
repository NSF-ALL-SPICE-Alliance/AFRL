...
```{r}
ggplot(all_data_good_models, aes(x=Var.Names, y=`MeanDecreaseAccuracy`)) +
  geom_segment( aes(x=Var.Names, xend=Var.Names, y=0, yend=`MeanDecreaseAccuracy`), color="skyblue") +
  geom_point(aes(size = MeanDecreaseGini), color="blue", alpha=0.6) +
  theme_light() +
  coord_flip() +
...
```{r}
explain_forest(rf_cell_cycle_and_proliferation_responses, interactions = TRUE, data = nmdata_no_na)
```



```{r}
plot_predict_interaction(rf_cell_cycle_and_proliferation_responses, nmdata_no_na, "exposure_duation_h", "core_size_nm")
```
